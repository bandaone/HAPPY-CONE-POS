...
- Owners and managers can maintain receipt identity and tax fields from **Settings → Receipt details**. The receipt extracts the configured VAT from VAT-inclusive prices and shows taxable sales and VAT in a compact tax section. It does not invent Smart Invoice/VSDC identifiers, fiscal signatures, SDC/MRC values, or verification QR codes.
...
- Final verification: 46 API tests and 32 web component tests passed; all 5 Playwright/Chrome journeys passed against the production build, including axe WCAG A/AA scans, 375 px reflow, editable stand and receipt details, the complete owner catalog workflow, all four staff-role boundaries, checkout, stock consumption, receipt and historical reprint, preparation, reporting, day close and offline recovery. Automated print-media evidence verifies that amount columns stay inside the receipt at 58 mm and 80 mm equivalent widths. TypeScript and Vite built successfully (360.70 kB JavaScript, 103.70 kB gzip; 47.44 kB CSS, 12.91 kB gzip). Migration replay on populated and partially upgraded SQLite databases, historic-order backfill and changed-file Python lint passed.
