...
## Catalog
- `GET /catalog` -> `{products:[{id,name,category,description,color,active,variants:[{id,name,price_ngwee,recipe:[{item_id,quantity}]}]}],modifiers:[{id,name,group,price_ngwee,active,recipe:[{item_id,quantity}]}]}`. Active catalog only. Seed: Vanilla, Chocolate, Strawberry with Single / Double variants; Cone/Cup serving options, Oreo/Sprinkles/Chocolate sauce toppings. Double Vanilla 3200 + Cone 500 + Oreo 500 = 4200. Require exactly one serving modifier for every order line; at most 3 toppings; no duplicate modifier IDs.
...
- Order `{id,number,business_day_id,status,created_at,lines,total_ngwee,payment:{method,status,amount_ngwee,tendered_ngwee,change_ngwee,provider,reference},refunded:boolean,refund_reason:null|string,offline:boolean}`. Lines same shape as quote. Payment status `CONFIRMED` or `REFUNDED`.
