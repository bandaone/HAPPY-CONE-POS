...
Owners and managers can also edit the stand profile from **Settings**. The dedicated **Receipt details** editor controls the legal name, shop or branch name, location, TPIN, contact number, tax category, VAT rate, and thank-you line printed on receipts. The remaining profile fields control display currency, timezone, payment and ticket instructions, activity introduction, and the counter guide. These values are stored in the database and used by the signed-in workspace, money and date display, offline cache, counter guide, and customer receipts. Live connection state, the signed-in identity, audit entries, and fiscal-integration status remain system controlled.
...
The system records cash and manual external-payment confirmations. Its printed document includes the saved business identity and an editable VAT-inclusive tax breakdown, but it does not claim Zambia Revenue Authority fiscal certification. Smart Invoice identifiers, signatures, verification QR codes, live mobile-money/card processing, and certified fiscal integration require separately approved integrations.
